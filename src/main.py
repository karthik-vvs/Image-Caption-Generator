...
dataset_text = os.path.join("data", "Flickr8k_text")
dataset_images = os.path.join("data", "Flicker8k_Dataset")
        ...
        while True:
            for key, description_list in descriptions.items():
                ...
                for i in range(len(input_image)):
                    yield {'input_1': input_image[i].astype(np.float32), 'input_2': input_sequence[i].astype(np.int32)}, output_word[i].astype(np.float32)
...
dataset = data_generator(train_descriptions, train_features, tokenizer, max_length, batch_size)
...
def get_steps_per_epoch(train_descriptions, batch_size):
    total_sequences = 0
    ...
    return max(1, (total_sequences + 31) // 32)
